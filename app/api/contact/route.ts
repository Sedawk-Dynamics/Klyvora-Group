...
export async function POST(request: Request) {
  ...
  // Validate email only if user entered one
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "The contact form is not configured. Please try again later.",
      },
      { status: 500 }
    )
  }
      ...
      <table style="border-collapse: collapse; width: 100%; margin-bottom: 16px;">
        ${rows
          .map(
            ([label, value]) => `
          <tr>
            <td style="padding: 8px 12px; background: #f4f6f9; font-weight: 600; width: 140px;">
              ${label}
            </td>
            <td style="padding: 8px 12px; border: 1px solid #e2e6ec;">
              ${escapeHtml(value)}
            </td>
          </tr>
        `
